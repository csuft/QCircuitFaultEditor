    ...
    : QObject(widget)
{
    ...
    m_sortType = NoSort;
    m_shape = shape;
    m_pointPen = QPen(QColor(255, 255, 255, 191), 1);		 // 绘制点的笔
    m_connectionPen = QPen(QColor(255, 255, 255, 127), 2);   // 点之间的连线
    m_pointBrush = QBrush(QColor(191, 191, 191, 127));
    m_pointSize = QSize(5, 5);  // 点的大小
    m_currentIndex = -1;
    m_editable = true;
    m_enabled = true;
...
bool HoverPoints::eventFilter(QObject *object, QEvent *event)
{
    if (object == m_widget && m_enabled) {
        ...
        case QEvent::MouseButtonPress:
        {
			...
			else if (me->button() == Qt::RightButton)  // 邮件移除浮点
			{
